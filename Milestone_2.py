    ...
    for column in df5.columns:
        ...
        for attack in condition:
            ...
            if df.dtypes[column] == 'object' or (
                ...
                df_5_conditioned[column].value_counts(normalize=True).plot(color='orange', kind='bar',
                                                                           label=f'Conditional for {attack}')
...
def print_summary(numerical_summary, categorical_summary):
    ...
    for column, info in numerical_summary.items():
        ...
        print(
            f"    Parameters: {info['params']}")  # First one is location parameter (Represents the central tendency or “location” of the distribution on the x-axis)
    ...
    for column, pmf in categorical_summary.items():
        ...
        for class_value, class_pmf in pmf.items():
            if class_value == 'overall':
                continue
            print(
                f"    PMF Conditioned on {class_value}:")  # Probabilities of each unique value given a specific class or condition.
