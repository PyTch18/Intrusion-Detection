...
#Task 2 part (I)
#to determine the best distribution we will use distfit as it trys the data on 89 different distributions
def best_fit_distribution_1(df4):
    dist = distfit()
    for column in df4.columns:
        if df4.dtypes[column] in ['int64', 'float64']:
            print(f"\nFitting distribution for column: {column}")
            try:
                # Filter data to exclude outliers (2nd to 98th percentiles)
                lower_bound = np.percentile(df4[column].dropna(), 2)
                upper_bound = np.percentile(df4[column].dropna(), 98)
                filtered_data = df4[(df4[column] >= lower_bound) & (df4[column] <= upper_bound)][column]
...
def plot_conditional_pdfs(df5):
    ...
    for column in numerical_columns:
        ...
        column_variance = df5[column].var()
        column_unique_values = df5[column].nunique()
        ...
        for value in class_values:
            ...
            data_conditioned = df5[df5['class'] == value][column].dropna()
...
def best_fit_mse(df6):
    # Select only numerical columns
    numerical_columns = df6.select_dtypes(include=[np.number]).columns

    # Get unique values in the 'class' column to condition on
    class_values = df6['class'].unique()
    ...
    for column in numerical_columns:
        ...
        adjust_range = has_extreme_values(df6[column])
        plt.figure(figsize=(12, 6))
        ...
        for value in class_values:
            # Filter the data for the current class value
            data_conditioned = df6[df6['class'] == value][column].dropna()
...
def pmf_plot(df7):
    for column in df7.columns:
        ...
        if df7.dtypes[column] == 'object' or (
                df7.dtypes[column] in ['int64', 'float64'] and
                (df7[column].var() < 1e-5 or df7[column].nunique() < 10)
        ):
            plt.figure(figsize=(10, 5))

            # Calculate and plot PMF using value_counts(normalize=True)
            pmf = df7[column].value_counts(normalize=True)
            pmf.plot(kind='bar')
    ...
    for column in df8.columns:
        ...
        for attack in condition:
            ...
            if df.dtypes[column] == 'object' or (
                ...
                plt.figure(figsize=(10,5))
                df8[column].value_counts(normalize=True).plot(color='blue', kind='bar', label='Original PMF')
